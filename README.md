...
一个面向运筹学入门、教学演示与个人作品展示的浏览器仿真：在同一组办公楼客流下，比较“协同 ETA 派梯 + 需求感知驻停”和朴素的“最近可达车 + 大厅驻停”。
...
本项目用平均等待、平均全程时间、最长等待、停靠次数与完成服务人数评价规则表现。简化约束包括：
...
点击“运行同样本 A/B 对比”后，系统用相同的随机种子和完全相同的乘客到达/起终点样本，分别独立运行两种策略 15 分钟，再比较：
...
优先级最高的扩展是：批量运行多个随机种子并展示置信区间；其次可将停靠 `Set` 升级为显式路线序列，比较 LOOK、最近车、分区和简单滚动时域插入策略。若继续面向研究，再考虑真实客流数据校准或用整数规划求解小规模离线基准。
